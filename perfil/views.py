...
def cadastrar_banco(request):
    ...
    if apelido.isdigit():
        messages.add_message(request, constants.ERROR, 'Preencha o campo APELIDO com texto apenas')
        return redirect('/perfil/gerenciar')
    
    if len(apelido) <= 2:
        messages.add_message(request, constants.WARNING, 'O campo APELIDO precisa ter no minimo 3 caracteres')
        return redirect('/perfil/gerenciar')
...
def cadastrar_categoria(request):
    ...
    if len(nome.strip()) == 0 or nome.isdigit():
        messages.add_message(request, constants.ERROR, 'Preencha o campo CATEGORIA com texto apenas')
        return redirect('/perfil/gerenciar')
    
    if len(nome) <= 2:
        messages.add_message(request, constants.WARNING, 'O campo CATEGORIA precisa ter no minimo 3 caracteres')
        return redirect('/perfil/gerenciar')
